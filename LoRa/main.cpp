...
String encrypt_message(const char *plaintext) {
  ...
  size_t padded_len = ((plain_len / 16) + 1) * 16;  // always adds 1–16 padding bytes (PKCS#7)
...
bool send_with_ack(const char *cipher_b64) {
  ...
  for (int attempt = 0; attempt < RETRY_LIMIT; attempt++) {
    ...
    // Put radio into non-blocking receive mode, then poll with millis() timeout
    radio.startReceive();
...
void loop() {
  ...
  if (M5Cardputer.Keyboard.isPressed()) {
    auto key = M5Cardputer.Keyboard.keysState(); // FIX: KeyState type renamed in M5Cardputer 1.1.x
    
    // Debounce: ignore keys pressed within 50ms of the last one (prevents double typing)
    if (millis() - lastKeyTime < 50) return;
    lastKeyTime = millis();
    
    // In M5Cardputer 1.1.x, KeysState uses boolean flags for special keys
    // and a std::vector<char> word for printable characters
    if (key.del) {
