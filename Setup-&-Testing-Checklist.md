...
| SPI Enabling               | `dtparam=spi=on` confirmed in `/boot/config.txt`. Pi rebooted.                                                                                                                                        |          | [x]       |
| Drivers / Test Environment | - Python environment setup (`python3 -m venv lora-test`).<br>- Required libraries installed (`pip install spidev RPi.GPIO` and `rpi-lora` if compatible).                                             |          | [x]       |
...
| Frequency Check            | Software configuration confirmed for 915MHz (or region’s ISM band).                                                                                                                                   |          | [x]       |
