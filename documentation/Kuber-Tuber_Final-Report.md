...
The following table summarises all 26 tests performed, their outcomes, and relevant notes. Detailed command outputs and timestamps are preserved in the project's test logs.
...
iptables \-A FORWARD \-s 10.0.20.0/24 \-d 10.0.10.94 \-p tcp \--dport 6443 \-j ACCEPT
...
Refer to the Glossary section below for definitions of all technical terms used, including LoRa, K3s, VLAN, AES-256-CBC, sequence number, replay protection, pod, node, and others.
...
**LoRa HAT** – A hardware add-on board (HAT stands for Hardware Attached on Top) that provides LoRa radio capability to a Raspberry Pi. The HAT connects to the Pi's GPIO pins and uses SPI for communication. The Waveshare E22-900T22S HAT used in this project is configured to operate at 868 MHz and includes an SX1262 transceiver chip. The HAT is attached to worker1 and serves as the LoRa gateway, receiving encrypted packets from the Cardputer and passing them to the bridge software. The HAT communicates with the Raspberry Pi via UART serial (/dev/ttyAMA0) at 9600 baud, not SPI. Mode-control pins GPIO22 (M0) and GPIO27 (M1) are used to switch the module between normal, configuration, and sleep modes.
